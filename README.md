...
There are ways you can handle more requests without increasing the RDS instance size. You can also optimize database queries so that they consume less resouces and time to get result. You can use in-memory cashing to cache less frequently updated data. You can get this data directly from cache and hence reducing requests to database. 
